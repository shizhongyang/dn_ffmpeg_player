...
JNIEXPORT void JNICALL Java_com_dongnaoedu_dnffmpegplayer_VideoUtils_decode(
	...
	//输出视频信息
	LOGI("视频文件格式：%s", pFormatCtx->iformat->name);
	LOGI("视频时长：%d", (pFormatCtx->duration)/1000000);
	LOGI("视频宽高：%d,%d", pCodecCtx->width, pCodecCtx->height);
	LOGI("解码器的名称：%s", pCodec->name);

	//准备读取
	//AVPacket 用于存储一帧一帧的压缩数据（H264）
